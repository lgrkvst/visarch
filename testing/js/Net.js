var Net = function () {
    ...
    var determineCenter = function () {
      center = Net.nodes.map(function (n) {
        return ix(n.id);
      });
      if (center.length > 3) {
        center.sort(function (a, b) {
          return nodes[a].weight > nodes[b].weight ? -1 : 1;
        });
    ...
    var lix = function (six, tix) {
      return links.filter(function (l) {
        return l.source === six && l.target === tix;
      });
    };
    var getNeighbors = function (id) {
      // returns duplicate NODES if two systems host multiple integrations
      var neighbor_links = links.filter(function (l) {
          return l.target.id === id || l.source.id === id;
        });
      var neighbor_nodes = neighbor_links.map(function (l) {
          return l.target.id === id ? l.source.id : l.target.id;
        });
      return neighbor_nodes.map(function (id) {
        return nodes[ix(id)];
      });
    };
    ...
    var add = function (n) {
      if (!node2links) {
        throw 'undefined callback: node2links';
        return;
      }
      if (!n.id) {
        throw 'node lacks id';
      ...
      if (typeof n.size === 'undefined') {
        n.size = 0;
        console.warn('Net added explicit size for node ' + n.name);
      }
      if (ix(n.id) < 0)
        var index = nodes.push(n) - 1;
      else
        return;
      // return if already among nodes
      if (dropLinks(n.id)) {
        throw 'Found garbage links to drop before adding node.';
      }
      node2links(n.id).forEach(function (l) {
        addLink(l);
      });
    ...
    var supernova = function (id) {
      // node explosion!
      node2links(id).forEach(function (l) {
        add(l.source);
        add(l.target);
      });
    };
